...
%   说明：旧 PlanarDrawApp.m 基于旧接口（runLArmIK_2D），本程序为重构版入口。
    ...
    s.hTaskType = uicontrol(tabScene,'Style','popupmenu', ...
        'String',{'move_to','move_along','move_for_pick','move_for_place','rotate','rotate_arm','facing_arm','pick','place','withdraw','reset','emergency_stop','cancel_task'}, ...
...
function onTaskRun(src, ~)
    ...
    ctList = {'move_to','move_along','move_for_pick','move_for_place','rotate','rotate_arm','facing_arm','pick','place','withdraw','reset','emergency_stop','cancel_task'};
...
function cmd = mockTaskCommand(ct, xt, yt, tt, dx, dy)
    cmd = struct();
    cmd.schema_version = '2.0';   % 与 TASK_COMMAND_INTERFACE v2.0 对齐
    ...
    cmd.selected_target = struct('target_id','TGT-GUI','class_name','payload_module', ...
        ...
        'pose_camera', struct('frame_id','camera_left', ...
            ...
            'orientation_euler', struct('roll',0,'pitch',0,'yaw',tt), ...   % 目标朝向 θ (rad)
    ...
    cmd.destination = struct('name','Assembly_Port_A', ...
        'pose_base', struct('frame_id','robot_base', ...
            ...
            'orientation_quat', struct('x',0,'y',0,'z',0,'w',1)));
    % 接口键（v2.0）：角度用度、距离用米、关节 1 索引、位置用 x/y；θ 由 UI 弧度转度
    cmd.params = struct('x', xt, 'y', yt, ...
        'theta_deg', rad2deg(tt), 'distance_m', 0.15, ...
        'alpha_deg', rad2deg(0.5), 'joint_index', 2, ...
        'destination', 'Assembly_Port_A');
    cmd.motion_params = cmd.params;   % 向后兼容镜像（内容与 params 相同）
