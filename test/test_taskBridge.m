...
%   构造 TaskCommand JSON → taskExecute → TaskStatus 回写验证
    ...
    fails = fails + assertTrue(numel(s.robot_state.joint_positions) == 6, 'joint_positions 回填');
...
%   用【可达绝对目标】(pose_base, frame robot_base)；move_to 另给 params.x/y（视觉系 x=右,y=前）。
    cmd = struct();
    ...
    cmd.selected_target = struct( ...
        ...
        'pose_base', struct('frame_id', 'robot_base', ...
            'position', struct('x', 1.0, 'y', 1.0, 'z', 0.0), ...
            'orientation_quat', struct('x', 0, 'y', 0, 'z', 0, 'w', 1)));
    if strcmp(ct, 'move_to'), cmd.params = struct('x', 1.0, 'y', 1.0); end
